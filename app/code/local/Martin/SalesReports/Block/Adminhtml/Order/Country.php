...
class Martin_SalesReports_Block_Adminhtml_Order_Country extends Mage_Adminhtml_Block_Template{
    ...
    public function __construct(array $args = array())
    {
        ...
        if($from = $this->helper('salesreports')->getParam('from')){
            ...
            $this->_from = $this->helper('salesreports')->convertDate($from,'en_US')->toString('Y-MM-dd HH:mm:ss');
        }else{
            $from = Mage::getModel('core/date')->date('m/d/Y',time());
            $this->_from = $this->helper('salesreports')->convertDate($from,'en_US')->subDay(7)->toString('Y-MM-dd HH:mm:ss');
        }
        ...
        if($to = $this->helper('salesreports')->getParam('to')){
            ...
            //$to = $to->format('Y-m-d');
            $this->_to = $this->helper('salesreports')->convertDate($to,'en_US')->addDay(1)->subSecond(1)->toString('Y-MM-dd HH:mm:ss');
        }else{
            $this->_to  = date('Y-m-d',time());
        }
    }
    ...
    public function getCollection(){
        ...
        $collection->getSelect()
            ...
            ->where('main_table.created_at>=?',array('from'=>$this->_from))
            ->where('main_table.created_at<?',array('to'=>$this->_to))
